<?php 
	...
	if(isset($_REQUEST['passwordBtn'])){
		$password=mysqli_real_escape_string($_REQUEST['newPassword']);
		$previous=mysqli_real_escape_string($_REQUEST['previousPassword']);
		$sql="SELECT password FROM user WHERE user_id='$user_id'";
		$result=mysqli_query($con,$sql) or die("Error");
		$rws= mysqli_fetch_array($result);
		if($rws[0]==$previous){
			$sql="UPDATE user SET password='$password' WHERE user_id='$user_id'";
			mysqli_query($con,$sql) or die("An error occurs.");
	...
	if(isset($_REQUEST['genderBtn'])){
		$gender=mysqli_real_escape_string($_REQUEST['gender']);
	...
	if(isset($_REQUEST['birthdayBtn'])){
		$birthday=mysqli_real_escape_string($_REQUEST['birthday']);
	...
	if(isset($_REQUEST['cityBtn'])){
		$city=mysqli_real_escape_string($_REQUEST['coordinate']);
